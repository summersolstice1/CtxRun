...
async fn copy_text_with_ignore_tag(text: String) -> Result<(), String> {
    // 写入剪贴板（Flag �理由 spotlight_paste 统一管理）
    let join_result = tauri::async_runtime::spawn_blocking(move || {
        ...
        // 2. 写入忽略标签（针对 Windows/Linux/各种管理器）
        // 写入一个空的自定义格式
        let _ = clipboard.set_buffer("ExcludeFromClipboardHistory", vec![0u8]);
        let _ = clipboard.set_buffer("NoCloudClipboard", vec![0u8]); // 禁止同步到 Windows 云剪贴板

        #[cfg(target_os = "macos")]
        {
            // macOS 专属忽略标签
            let _ = clipboard.set_buffer("org.nspasteboard.AutoGeneratedType", vec![0u8]);
            let _ = clipboard.set_buffer("org.nspasteboard.ConcealedType", vec![0u8]);
        }
...
async fn copy_image_with_ignore_tag(image_path: String) -> Result<(), String> {
    ...
    let join_result = tauri::async_runtime::spawn_blocking(move || {
        ...
        // 写入忽略标签（针对 Windows/Linux/各种管理器）
        let _ = clipboard.set_buffer("ExcludeFromClipboardHistory", vec![0u8]);
        let _ = clipboard.set_buffer("NoCloudClipboard", vec![0u8]);
...
pub async fn spotlight_paste(
    ...
    // 2. 从数据库获取内容 (使用独立代码块，确保 MutexGuard 在 await 前释放)
    let (kind, content) = {
        let conn = state.conn.lock().map_err(|e| e.to_string())?;
    ...
    // 3. 先隐藏窗口
    // 隐藏窗口能立即把焦点还给上一个应用，给它充足的时间准备接收输入
    if let Some(window) = app.get_webview_window("spotlight") {
        window.hide().map_err(|e| e.to_string())?;
    }

    // 4. 写入剪贴板 (带忽略标签，移除内部的 sleep 和 store(false))
    if kind == "image" {
    ...
    // 5. 异步执行按键模拟并延迟重置 Flag
    tauri::async_runtime::spawn(async move {
        // 关键延迟：等待窗口动画结束，焦点完全切换回目标应用
        // 150ms 适用于较慢的软件（如 Java Swing 应用或旧版 Office）
