...
impl RefineryHandler {
    ...
    /// 检查剪贴板内容是否带有"忽略"标签（工业标准 MIME 标记法）
    /// 参考：Raycast、CopyQ、1Password 等工具的实现
    fn should_ignore(&self, ctx: &ClipboardContext) -> bool {
        // 检查是否处于粘贴状态（我们自己正在粘贴）
        if self.is_pasting.load(std::sync::atomic::Ordering::Relaxed) {
            return true;
        }

        // 定义工业界公认的"忽略/隐私"标签
        let ignore_formats = [
            "ExcludeFromClipboardHistory",    // 通用/Windows
            "org.nspasteboard.AutoGeneratedType", // macOS 专用
            "org.nspasteboard.ConcealedType",    // macOS 敏感数据
            "NoCloudClipboard",               // Windows 云剪贴板忽略
        ];

        // 检查可用的格式
        if let Ok(available) = ctx.available_formats() {
            for format in available {
                let format_str = format.as_str();
                if ignore_formats.iter().any(|ignore| format_str.contains(*ignore)) {
                    println!("[Refinery] Skipping ignored format: {}", format_str);
                    return true;
                }
            }
        }
