...
mod tests {
    ...
    use super::{list_dir_slice, parse_rg_results, read_file_lines};

    fn temp_root() -> PathBuf {
        let root =
            std::env::temp_dir().join(format!("ctxrun-tool-runtime-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(&root).expect("create temp root");
        root
    }

    #[tokio::test]
    async fn read_file_lines_respects_offset_and_limit() {
        let root = temp_root();
        let path = root.join("sample.txt");
        fs::write(&path, "a\nb\nc\n").expect("write sample");

        let (lines, truncated) = read_file_lines(&path, 2, 2).await.expect("read lines");
    ...
    #[tokio::test]
    async fn list_dir_slice_respects_depth_and_paging() {
        let root = temp_root();
        ...
        let (entries, total, truncated) = list_dir_slice(&root, Path::new(""), 1, 10, 2)
            .await
            .expect("list dir");
        assert_eq!(
            entries,
    ...
    #[test]
    fn parse_rg_results_applies_limit() {
        let parsed = parse_rg_results(b"a.rs\nb.rs\nc.rs\n", 2);
